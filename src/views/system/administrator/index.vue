...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listAdministrator(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
    ...
    handleStatusChange(row) {
      ...
      this.$confirm('确认要"' + text + '""' + row.user_name + '"用户吗?', '警告', {
        ...
        return changeAdministratorStatus(row.id, row.status)
      }).then(() => {
    ...
    handleAdd() {
      this.reset()
      getAdministrator().then(response => {
    ...
    handleUpdate(row) {
      this.reset()
      const userId = row.id
      getAdministrator(userId).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.form.id !== undefined) {
            updateAdministrator(this.form).then(response => {
              if (response.code === 0) {
            ...
            addAdministrator(this.form).then(response => {
              if (response.code === 0) {
